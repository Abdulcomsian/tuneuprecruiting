                                    ...
                                    <tbody>
                                        @foreach ($programs as $program)
                                            ...
                                            <tr class="border-bottom-secondary {{ $formatting['bgColor'] }}">
                                                <td class="{{ $formatting['bgColor'] }}">
                                                    {{ $program->college_or_university }}</td>
                                                <td class="{{ $formatting['bgColor'] }}">
                                                ...
                                                <td>
                                                    <ul class="action">
                                                        <x-student-additional-requirement-button :apply="$applyDetails"
                                                            :formatting="$formatting" :userId="$program->user_id" />
                                                        <x-student-chat-button :apply="$applyDetails" :formatting="$formatting"
                                                            :userId="$program->user_id" />

                                                        <li class="edit">
                                                            <a class="{{ $formatting['textColor'] }} btn-view"
                                                                title="View"
                                                                data-route="{{ url('program/details/' . encrypt($program->id)) }}">
                                                                <i
                                                                    class="icofont icofont-eye-alt {{ $formatting['textColor'] }}"></i></a>
                                                        </li>
                                                        @if (empty($applyDetails))
                                                            <li class="edit">
                                                                <a class="{{ $formatting['textColor'] }}"
                                                                    ...
                                                                    <i
                                                                        class="icofont icofont-law-document {{ $formatting['textColor'] }}"></i>
                                                                </a>
                                                            </li>
                                                        @endif
                                                    </ul>
                    ...
                    <div class="modal-body dark-modal">
                        ...
                        <form method="POST" id="frm-program" action="{{ route('program.store') }}"
                            ...
                            <div class="col-md-4">
                                ...
                                <input class="form-control program-name" id="validationCustom01" type="text"
                                    name="program_name" placeholder="Enter program name" required="">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label" for="validationCustom02">Session</label>
                                <input class="form-control session" id="validationCustom02" type="text"
                                    name="session" placeholder="Enter session" required="">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label" for="validationCustom02">Number of Students</label>
                                <input class="form-control number-of-students" id="validationCustom02" type="text"
                                    name="number_of_students" placeholder="Enter number" required="">
                            </div>
                            <div class="col-md-12">
                                <div class="mb-3">
    ...
    <script>
        ...
        function createProgramHTML(data) {
            ...
            if (data.program.apply == 'Applied') {
                htmlContent += '<a href="' + data.program.applyDetailRoute +
                    '" class="btn bg-info mt-4 float-end ml-3" type="button">Apply Details</a>'
                htmlContent +=
                    '<a href="#"  class="btn btn-default mt-4 float-end disabled" disabled type="button">Applied</a>'
            } else {
                htmlContent += '<a href="' + data.program.applyRoute +
                    '" class="btn bg-primary mt-4 float-end" type="button">Apply Now</a>'
            }
            return htmlContent;
        }
